...
| `does not support schemas` | Missing `supports_schema: true` in extra-openai-models.yaml | Add the flag — see capability flags below |
...
### Capability flags for extra-openai-models.yaml

These boolean flags tell `llm` what the model supports. Without them, `llm` will reject the corresponding feature with an error.

| Flag | Enables |
| --- | --- |
| `supports_tools: true` | `--tool` / `-T` tool use |
| `supports_schema: true` | `--schema` structured JSON output |
| `vision: true` | Image attachments (`-a image.jpg`) |
| `audio: true` | Audio attachments |
| `reasoning: true` | Reasoning models (o1, o3-mini style) |
| `completion: true` | Completion (non-chat) models |
