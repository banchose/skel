...
getwet() {
  ...
  local LAT=$(echo "${OPENWEATHER_LOCATION}" | jq -r '.lat')
  local LON=$(echo "${OPENWEATHER_LOCATION}" | jq -r '.lon')
  ...
  local RESPONSE=$(curl -s -f "https://api.openweathermap.org/data/3.0/onecall?lat=${LAT}&lon=${LON}&exclude=minutely,hourly,daily,alerts&appid=${OPENWEATHER_APP_ID}&units=metric")
  ...
  if echo "${RESPONSE}" | jq -e 'has("cod") and (.cod | tostring | startswith("4") or startswith("5"))' &>/dev/null; then
    ...
    local ERROR_CODE=$(echo "${RESPONSE}" | jq -r '.cod')
...
getwet2() {
  ...
  local LOCATION="latitude=42.742830&longitude=-73.801163"
  local CURRENT="current=temperature_2m,..."
  local HOURLY="hourly=temperature_2m,..."
  local DAILY="daily=weather_code,..."
  local OPTIONS="past_days=3&forecast_days=5&timezone=America/New_York&temperature_unit=fahrenheit"
