...
| `brs` / `brsn` / `brss` / `brsT` | LiteLLM → Bedrock | `brs` | Distinct from native `bedrock-sonnet` |
...
*   Bare alias (e.g., `bro`): `-T Exa -T simple_eval -T llm_version -T llm_time -T get_answer -T get_contents`
    
*   `s` suffix (e.g., `bros`): `-T web_search` instead of `-T Exa`, rest same
...
7.  **Functions:**
    ...
    *   `llm_status` — health check: model, keys, env vars, plugins, logging, litellm proxy with `==>` issue markers and summary
