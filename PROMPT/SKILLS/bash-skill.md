...
After writing or editing any script, before reporting it done:
```bash
bash -n script.sh                 # syntax check
shellcheck -S style script.sh     # fix everything it reports, no exceptions
```
If `shellcheck` isn't installed, say so rather than skipping the step. Disable a
check only with an inline `# shellcheck disable=SCxxxx` plus a comment saying why.
...
require() {
    ...
    if (( ${#missing[@]} )); then
        # [*] not [@]: printf reuses its format string, so "${missing[@]}"
        # would pair missing[1] with the %s meant for the script name
...
| IP address | `ip`, `ifconfig` | `hostname -I` (external — see caveat below) |
...
- `hostname` is itself an external binary (net-tools/inetutils) and is missing from many minimal images — only `/dev/tcp`, `getent`, and `/proc` reads are guaranteed without extra packages
