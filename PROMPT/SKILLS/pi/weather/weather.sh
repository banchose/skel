...
fetch() {
  ...
  safe=${url##*/onecall/}
  safe=${safe%%\?*} # path only - never echo appid
  raw=$(curl -s --max-time 15 -w $'\n%{http_code}' "$url") || {
    echo "CURL FAILED: $safe" >&2
    return 1
  }
  code=${raw##*$'\n'}
  body=${raw%$'\n'*}
  if [ "$code" != 200 ]; then
    local safe=${url##*/onecall/}
    safe=${safe%%\?*} # never echo appid
...
[ -n "$cur$om" ] || {
  echo "no data from either source (see errors above)" >&2
  exit 1
}
...
if [ "$(printf '%s' "$alerts" | jq 'length')" != 0 ]; then
  echo
  echo "ALERTS (national agency, via OWM)"
