...
# NAME="${IMAGE%:*}0"
NAME="socat-proxy0"
COMMAND='socat TCP-LISTEN:80,fork,reuseaddr TCP:nginx0:80'

# docker run --rm -it --network socat-proxy0 --network nginx0 nicolaka/netshoot socat TCP-LISTEN:80,fork,reuseaddr TCP:nginx0:80
...
docker run -it -d --rm --hostname "${NAME}" --name "${NAME}" --network nginx0 --network "${NAME}" "${IMAGE}" ${COMMAND}
